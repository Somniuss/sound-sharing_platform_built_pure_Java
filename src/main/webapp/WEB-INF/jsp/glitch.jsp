...
/* Настраиваем кастомный стиль плеера */
audio::-webkit-media-controls-panel {
	background-color: #000 !important;
	color: white !important;
	height: 32px;
	border-radius: 5px; /* Делаем углы аккуратными */
}
...
audio::-webkit-media-controls-play-button, audio::-webkit-media-controls-pause-button,
	...
	filter: brightness(0) invert(1); /* Белые кнопки */
}

audio::-webkit-media-controls-timeline, audio::-webkit-media-controls-volume-slider
	{
	filter: brightness(1) invert(0); /* Линия таймлайна остается темной */
}
...
/* Стиль для кнопки "Back to Main Page" */
.btn-back-to-main {
	width: 200px;
	padding: 10px 18px;
	border: none;
...
.btn-back-to-main:active {
	background-color: #990000; /* Еще темнее при нажатии */
	transform: scale(1);
}
...
<body>
	...
	<div class="content">
		<%
            List<Sound> glitchSounds = (List<Sound>) request.getAttribute("glitchSounds");
            if (glitchSounds != null && !glitchSounds.isEmpty()) {
                for (Sound sound : glitchSounds) {
        %>
        ...
        %>
		<p>No glitch sounds available.</p>
		<% } %>
	</div>

	<!-- Кнопка "Back to Main Page" -->
